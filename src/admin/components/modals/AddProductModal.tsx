...
export default function AddProductModal({ open, onClose, onSaved }: AddProductModalProps) {
  const [form, setForm] = useState({
    name: '', categoryId: '', purity: '22K', weight: '', weightGrams: '',
    price: '', description: '', makingStyle: '',
    isFeatured: false, isAvailable: true, isNewArrival: false, isSoldOut: false, isHidden: false,
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fd.append('isSoldOut', String(form.isSoldOut));
      ...
      onSaved?.();
      onClose();
      setForm({ name: '', categoryId: '', purity: '22K', weight: '', weightGrams: '', price: '', description: '', makingStyle: '', isFeatured: false, isAvailable: true, isNewArrival: false, isSoldOut: false, isHidden: false });
      setImages([]);
    } catch (err: any) {
            ...
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', overflow: 'hidden', flex: 1, minHeight: 0 }}>
                ...
                <div style={{ padding: '28px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '18px' }}>
                  {error && <div style={{ padding: '10px 14px', backgroundColor: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: '8px', fontFamily: 'var(--font-body)', fontSize: '0.8125rem', color: 'var(--admin-danger)' }}>{error}</div>}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '18px' }}>
                    ...
                    <FormField
                      as="select" label="Category" required value={form.categoryId}
                      onChange={(e) => set('categoryId', e.target.value)}
                      options={[{ value: '', label: 'Select category' }, ...categories.map((c: any) => ({ value: c.id, label: c.name }))]}
                    />
                    <FormField
                      as="select" label="Purity" required value={form.purity}
                    ...
                    <FormField
                      as="select" label="Making Style" value={form.makingStyle}
                  ...
                  <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                    ...
                    <Toggle label="Available" value={form.isAvailable} onChange={(v) => set('isAvailable', v)} />
                    <Toggle label="Sold Out" value={form.isSoldOut} onChange={(v) => set('isSoldOut', v)} />
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', padding: '16px 28px', borderTop: '1px solid var(--admin-border)', backgroundColor: 'var(--admin-bg)', flexShrink: 0 }}>
                <AdminButton variant="secondary" onClick={onClose} disabled={saving}>Cancel</AdminButton>
  ...
  return (
    <div>
      ...
      <button
        ...
        style={{
          width: '44px', height: '24px', borderRadius: '100px', border: 'none', cursor: 'pointer', padding: '2px',
          backgroundColor: value ? 'var(--color-gold)' : 'var(--admin-border)', transition: 'background-color 0.25s',
          display: 'flex', alignItems: 'center',
        }}
      >
        <div style={{ width: '20px', height: '20px', borderRadius: '50%', backgroundColor: '#fff', transform: value ? 'translateX(20px)' : 'translateX(0)', transition: 'transform 0.25s', boxShadow: '0 1px 4px rgba(0,0,0,0.2)' }} />
      </button>
    </div>
  );
}
